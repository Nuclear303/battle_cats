...
function getEnemy(){
    ...
    if(rnd == regular.length){
        ...
        localStorage.setItem("enemy",JSON.stringify(bosses[rnd]));
    }
    else{
        localStorage.setItem("enemy",JSON.stringify(regular[rnd]));
        ...
        team.forEach(cat=>{
            if(cat != null){
                ...
                button.addEventListener("click", _=> {
                    localStorage.setItem("chosenCat", JSON.stringify(cat));
                    document.querySelector("main").innerHTML = "<div class='arena'></div><button class='nextTurn'>Next Turn!</button>"
                    let chosenCat = new Cat("Cat",100,8,"None","Cat.png");
                    let enemy = new Enemy("Doge",90,8,1,"None", 10,"Doge.png", false);
                    // let chosenCat = JSON.parse(localStorage.getItem("chosenCat"));
